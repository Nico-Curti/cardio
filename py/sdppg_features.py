...
def find_first_index_for_maximum_in_zero_crossing(sdppg, zero_crossing):
    ...
    Parameters
    ----
    sdppg: 1D array-life of float; the sdppg signal
    zero_crossing: 1D array-like of float; the zero crossing found for sdppg through its second derivative

    Returns
    ----
    start: int; leftomst end of the interval containing a local maximum
    """
...
def find_next_a(sdppg, zero_crossing, start):
    ...
    Notes: if there is no next a, a is set to np.nan and k ito len(zero_crossing)
...
def features_from_sdppg(t, signal, l=100, flip=True):
    """
    ...
    l: int; factor used to compute the new number of points: the length of the splined signal will be len(t)*l
    flip: bool; should the signal be flipped?
    NOTES: t and signal must have the same length; t must be monotonic and positive

    Returns
    ----
    a dictionary containing a, b, c, d, e: the features extracted from the SDPPG.
    ...
    newt = np.linspace(t[0], t[-1], len(t)*l)
    spline_sdppg = spline_sdppg(newt)  # TODO change name?
    spline_fdppg = spline_fdppg(newt)
    # normalisation
    spline_sdppg /= np.max(spline_sdppg)
    spline_fdppg /= np.max(spline_fdppg)
    # flip
    if flip:
        spline_sdppg = np.flip(spline_sdppg)
        spline_fdppg = np.flip(spline_fdppg)
    # find the zero crossings
    intermediate_step = spline_fdppg[0:-1]*spline_fdppg[1:]
    zero_crossing = np.where(intermediate_step < 0.)[0]
    # find starting position
    i = find_first_index_for_maximum_in_zero_crossing(
            spline_sdppg, zero_crossing)
    a = []
    b = []
    c = []
    d = []
    ...
    z_len = len(zero_crossing)
    while i < z_len:
        next_a, i = find_next_a(spline_sdppg, zero_crossing, i)
        if i + 4 >= z_len or next_a == np.nan:
            break

        next_b = np.min(spline_sdppg[zero_crossing[i]:zero_crossing[i+1]])
        next_c = np.max(spline_sdppg[zero_crossing[i+1]:zero_crossing[i+2]])
        next_d = np.min(spline_sdppg[zero_crossing[i+2]:zero_crossing[i+3]])
        next_e = np.max(spline_sdppg[zero_crossing[i+3]:zero_crossing[i+4]])
        # No need to check c: it's the next maximum after a; always < a
        if next_e >= next_a:
            i += 3  # Find the next 'a' after the bad region due to e
        else:
            a.append(next_a)
            b.append(next_b)
            c.append(next_c)
            d.append(next_d)
            e.append(next_e)
            i += 5  # i+4 -> i+5 should have a minimum inside
